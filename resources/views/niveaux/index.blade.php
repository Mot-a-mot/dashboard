            ...
            <div class="col-sm-12">
               <div class="iq-card">
                  <div class="iq-card-header d-flex justify-content-between">
                     ...
                     <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#createNiveauModal">
                        Ajouter un Niveau
                     </button>
                           ...
                           <tbody>
                              @forelse ($niveaux as $index => $niveau)
                                 <tr>
                                    ...
                                    <td>{{ $niveau->name }}</td>
                                    <td>{{ $niveau->order }}</td>
                                    <td>{{ $niveau->created_at->format('Y-m-d') }}</td>
                                    <td>
                                       <a href="{{ route('niveaux.edit', $niveau->id) }}" class="btn btn-warning btn-sm">Modifier</a>
                                       <form action="{{ route('niveaux.destroy', $niveau->id) }}" method="POST" style="display:inline-block;" onsubmit="return confirm('Supprimer ce niveau ?');">
                                          @csrf
                                          <a href="{{ route('exercices.byNiveau', ['niveau_id' => $niveau->id]) }}" class="btn btn-info btn-sm">Voir Exercices</a>
                                       </form>
                                    </td>
                                 </tr>
                              @empty
